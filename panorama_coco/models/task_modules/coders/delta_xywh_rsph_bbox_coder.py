...
class DeltaXYWHSphBBoxCoder(BaseBBoxCoder):
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        assert bboxes.size(-1) == gt_bboxes.size(-1)
        return bbox2delta(bboxes, gt_bboxes, self.means, self.stds, self.delta_ratio)
...
def delta2bbox(
    ...
    """
    box_dim = 4
    num_bboxes, num_classes = deltas.size(0), deltas.size(1) // box_dim
    if num_bboxes == 0:
        return deltas

    deltas = deltas.reshape(-1, box_dim)
