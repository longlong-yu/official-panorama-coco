...
from panorama_coco.ops.diff_iou_rotated import diff_iou_rotated_2d
from panorama_coco.registry import MODELS
from panorama_coco.sdk.sph2pob.bbox.iou.sph2pob_efficient import sph2pob_efficient
from panorama_coco.utils.bbox import jiter_rotated_bboxes, jiter_spherical_bboxes, r2hbox_xyxy
from panorama_coco.utils.spherical import FLOAT_EPS


class Sph2ObbTransfrom:
    def __init__(self, transform='xinyuan'):
        assert transform in ['xinyuan']
        if transform =='xinyuan':
            self.transform = sph2pob_efficient
        else:
    ...
    def __call__(self, cls):
        ...
        def new_forward(_self_, pred, target, weight=None, *args, **kwargs):
            box_version = pred.shape[-1]
            pred, target = pred.clone(), target[..., :box_version].clone()
            ...
            pred[..., 1] = torch.pi / 2 - pred[..., 1]
            target[..., 0] = target[..., 0] + torch.pi
            target[..., 1] = torch.pi / 2 - target[..., 1]
            
            pred, target = jiter_spherical_bboxes(pred, target)
            #pred, target = climp_spherical_boxes(pred, target)
            
            pred, target = self.transform(pred, target, rbb_angle_version='rad')
            
            pred, target = jiter_rotated_bboxes(pred, target)
            #pred, target = climp_rotated_boxes(pred, target)
...
class OBBIoULoss(nn.Module):
    def __init__(self, mode='iou', eps=FLOAT_EPS, reduction='mean', loss_weight=1.0):
    ...
    def forward(self,
        ...
        return self.loss_weight * obb_iou_loss(
            pred,
            target,
            weight,
            mode=self.mode,
...
@weighted_loss
def obb_iou_loss(pred, target, mode='iou', eps=FLOAT_EPS):
    ...
    hbb_pred = r2hbox_xyxy(pred)
    hbb_target = r2hbox_xyxy(target)
...
@Sph2ObbTransfrom()
class SphIoULoss(RotatedIoULoss):
    """SphRotatedIoULoss.
...
@MODELS.register_module()
@Sph2ObbTransfrom()
class SphOBBIoULoss(OBBIoULoss):
    """SphOBBIoULoss.
