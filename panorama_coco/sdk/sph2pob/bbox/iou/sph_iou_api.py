...
def _sph2pob_iou_auxiliary(bboxes1, bboxes2, transform, mode, is_aligned, calculator, rbb_edge, rbb_angle):
    ...
    # !Note: To prevent excessive memory usage, limit the slice size when is_aligned = False.
    max_size = 256
    if not is_aligned and cols > 1e5 and rows > max_size:
        split_bboxes1 = bboxes1.chunk(max_size)
        overlaps_list = []
        for item in split_bboxes1:
            ret = _inner_sph2pob_iou_auxiliary(
                bboxes1=item,
                bboxes2=bboxes2,
                transform=transform,
                mode=mode,
            ...
            overlaps_list.append(ret)
        return torch.cat(overlaps_list) 
    else:
        return _inner_sph2pob_iou_auxiliary(
            bboxes1=bboxes1,
            bboxes2=bboxes2,
            transform=transform,
            mode=mode,
...
def _inner_sph2pob_iou_auxiliary(bboxes1, bboxes2, transform, mode, is_aligned, calculator, rbb_edge, rbb_angle):
    rows = bboxes1.size(0)
    cols = bboxes2.size(0)
    if not is_aligned:
    ...
    return overlaps.clamp_(min=0, max=1)
...
def naive_iou(bboxes1, bboxes2, mode='iou', is_aligned=False, box_formator='sph2pix'):
    ...
    # 弧度变为角度
    bboxes1 = torch.rad2deg(bboxes1)
    bboxes2 = torch.rad2deg(bboxes2)
    ...
    # 角度变为弧度
    bboxes1 = torch.deg2rad(bboxes1)
    bboxes2 = torch.deg2rad(bboxes2) 
