...
from panorama_coco.lib.equilib.grid_sample import torch_grid_sample
from panorama_coco.lib.equilib.torch_utils import get_device
...
def _equirect_facetype(h: int, w: int, device: torch.device = torch.device("cpu")) -> torch.Tensor:
    """0F 1R 2B 3L 4U 5D"""
    ...
    tp = torch.roll(
        torch.arange(4, device=device)  # 1
    ...
    idx = torch.linspace(-(math.pi * w_ratio), math.pi * w_ratio, w // 4, device=device) / 4
    idx = h // 2 - torch.round(
...
def create_equi_grid(
    ...
    tp = _equirect_facetype(h_out, w_out, device=device)
    ...
    grid = torch.stack((coor_y, coor_x), dim=-3)
    return grid
